...
no_files_df['No_Missing'] = no_files_df['No_Files'].apply(lambda x : int(x - median_files))
...
# Media augmentation is used only in species which the number of files is below the mean
...
for k in range(0,no_files_df.shape[0]):
    ...
    if nf < 0:
        ...
        for i in range(0, nf):
            ...
            augmented_signal = librosa.effects.time_stretch(signal, rate=np.random.random_sample())
...
for b in birds:   # only selected birds for the competition
    ...
    cnt = 0
    for filename in os.listdir(f'./train_audio/{b}'):
        if cnt < 5:
            # Sound file is loaded
            soundname = f'./train_audio/{b}/{filename}'
            ...
            mel_spct = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=128)
...
birds_fe.to_csv(r'./birds_fe.csv', index = False)
